...
class ReplyRepositoryPostgres extends ReplyRepository {
  ...
  async getCommentReplies(commentId) {
    const query = {
      text: 'SELECT a.id, b.username, a.inserted_at AS date, (CASE WHEN a.deleted_at IS NULL then a.content ELSE \'**balasan telah dihapus**\' END) AS content FROM replies a JOIN users b ON a.owner_id = b.id WHERE a.comment_id = $1 ORDER BY a.inserted_at ASC',
      values: [commentId],
    };
    const result = await this._pool.query(query);
    return result.rows;
